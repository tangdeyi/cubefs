...
var (
	InvalidModeErr                 = fmt.Errorf("Invalid Retention Mode")
	RetentionPeriodsTooLargeErr    = fmt.Errorf("Default retention period is too large")
	NonPositiveRetentionPeriodsErr = fmt.Errorf("Default retention period must be a positive integer value")
	BothDaysAndYearsSpecifiedErr   = fmt.Errorf("Either Days or Years must be specified, cannot be specified both at the same time")
	EitherDaysOrYearsSpecifiedErr  = fmt.Errorf("Either Days or Years must be specified")
	InvalidObjectLockEnabledErr    = fmt.Errorf("Only Enabled value is allowd to ObjectLockEnabled element")
	NilDefaultRetentionErr         = fmt.Errorf("Default retention cannot be nil")
	NilDefaultRuleErr              = fmt.Errorf("Default rule cannot be nil")
)
