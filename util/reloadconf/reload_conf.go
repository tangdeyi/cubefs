...
func (self *ReloadConf) remoteReload(reload func(data []byte) error) (err error) {
	...
	if bytes.Equal(md5sum, self.md5sum) {
		log.LogDebugf("remoteReload:", self.ConfName, "do nothing cause of md5sum is equal")
		return nil
	}
...
func (self *ReloadConf) localReload(reload func(data []byte) error) (err error) {
	...
	if bytes.Equal(md5sum, self.md5sum) {
		log.LogDebugf("localReload:", self.ConfName, "do nothing cause of md5sum is equal")
		return nil
	}

	log.LogInfo("localReload: %v local file is changed, oldmd5: %v, newmd5: %v", self.ConfName, self.md5sum, md5sum)
	err = reload(data)
	if err != nil {
